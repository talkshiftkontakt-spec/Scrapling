...
class LapaNinjaProvider(SourceProvider):
    slug = "lapa_ninja"
    source_url = "https://www.lapa.ninja/"

    def discover(self) -> list[DiscoveredWebsiteRecord]:
        page = self.fetch_catalog()
        records: list[DiscoveredWebsiteRecord] = []
        for card in page.css("a[href*='/site/']"):
            target = card.attrib.get("href")
            if not target:
                continue
            title = card.css("img::attr(alt)").get("Lapa Ninja reference").strip()
            records.append(
