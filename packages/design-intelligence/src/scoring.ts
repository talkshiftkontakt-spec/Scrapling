...
export class QualityScoringService {
  public score(analysis: AnalysisResult): QualityScore {
    ...
    const premiumFeel = analysis.designKeywords.some((keyword) => keyword.toLowerCase().includes("premium")) ? 8.8 : 7.1;
    ...
    const originality = analysis.designKeywords.some((keyword) => keyword.toLowerCase().includes("generic")) ? 5.8 : 8.0;
